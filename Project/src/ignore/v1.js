...
let currentOrientation = 'unknown'; // This can be 'left', 'right', or 'unknown'
...
async function getAccel() {
  DeviceMotionEvent.requestPermission().then((response) => {
    ...
    //     let newOrientation = (x < -9) ? 'left' : (x > 9) ? 'right' : 'unknown';
    //     if (newOrientation !== 'unknown' && newOrientation !== currentOrientation) {
    //       currentOrientation = newOrientation;
    //       lastOrientationChangeTime = Date.now();
    //     }

    //     // Check if the device has been in the current orientation for at least 3 seconds
    //     if ((Date.now() - lastOrientationChangeTime) >= 3000) {
    //       // Reset the timer
    //       lastOrientationChangeTime = Date.now();

    //       // Move the carousel based on the orientation
    //       if (currentOrientation === 'left') {
    //         moveSlide(-1); // Move to the previous photo
    //       } else if (currentOrientation === 'right') {
    //         moveSlide(1); // Move to the next photo
    //       }

    //       // Reset orientation to prevent continuous slide changes
